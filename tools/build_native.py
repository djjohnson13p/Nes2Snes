...
QUICK_INDEXED = frozenset(('LDA','STA','AND','ORA','EOR','ADC','SBC','CMP'))

QUICK_ZPX = frozenset(('LDA','LDY','STA','STY','AND','ORA','EOR','ADC','SBC','CMP','ASL','LSR','ROL','ROR','INC','DEC'))
...
def build(rom_path:Path,trace:Path,out:Path,fastrom:bool=True,quick_zp:bool=True,quick_indirect:bool=True,quick_io:bool=True,quick_ppu:bool=True,object_cache:bool=True,pipelined_video:bool=True,replay_input:Path|None=None,direct:bool=True,unrolled_objects:bool=True,experimental_audio:bool=False,stress_nmi_restore:bool=False,safe_addresses:bool=True,simple_direct:bool=True,bank_direct:bool=True,stress_bank_switch:bool=False,specialized_indirect:bool=True,audio_counters:bool=False,raster_scroll:bool=False,layer_masks:bool=True,audio_sweep:bool=False,quick_indexed:bool=True,word_oam:bool=True,runtime_counters:bool=True):
    ...
    if audio_sweep and not audio_counters:
        raise ValueError("--audio-sweep requires --audio-counters and --experimental-audio")
    ...
    if audio_sweep:
        audio_info["scope"]="Four-voice DSP with quantized envelopes, lengths, linear counter and pulse sweep; no DMC or cycle-accurate timing."
    nmi,reset,irq=struct.unpack_from('<HHH',rom.prg,0x3fffa)
    (assets/'config.inc').write_text(f'USE_RUNTIME_COUNTERS={int(runtime_counters)}\nUSE_WORD_OAM={int(word_oam)}\nUSE_QUICK_INDEXED={int(quick_indexed)}\nUSE_AUDIO_SWEEP={int(audio_sweep)}\nUSE_LAYER_MASKS={int(layer_masks)}\nUSE_RASTER_SCROLL={int(raster_scroll)}\nUSE_AUDIO_COUNTERS={int(audio_counters)}\nUSE_SPECIALIZED_INDIRECT_LDA={int(specialized_indirect)}\nTEST_NMI_RESTORE_STRESS={int(stress_nmi_restore)}\nUSE_EXPERIMENTAL_AUDIO={int(experimental_audio)}\nGUEST_NMI=${nmi:04X}\nGUEST_RESET=${reset:04X}\nGUEST_IRQ=${irq:04X}\nUSE_FASTROM={int(fastrom)}\nUSE_DIRECT_CALLS={int(direct)}\nUSE_PIPELINED_VIDEO={int(pipelined_video)}\nUSE_OBJECT_CACHE={int(object_cache)}\nUSE_UNROLLED_OBJECTS={int(unrolled_objects and object_cache)}\nTEST_INPUT_REPLAY={int(replay_input is not None)}\nREPLAY_LENGTH={len(replay)}\nUSE_QUICK_PPU={int(quick_ppu and quick_io)}\nUSE_QUICK_DISPATCH={int(quick_zp or quick_indirect or quick_io or quick_indexed)}\n')
    ...
    for opcode in range(256):
        ...
        if quick_indexed and mode in ('absx','absy') and name in QUICK_INDEXED:
            quick.append('Quick'+name+'_'+mode.upper())
        elif quick_zp and mode=='zpx' and name in QUICK_ZPX:
...
if __name__=='__main__':
    ...
    p.add_argument('--no-layer-masks',action='store_true',help='Retain the previous left-edge and top-mask behavior for negative-control tests')
    p.add_argument('--experimental-raster-scroll',action='store_true',help='Opt-in IRQ PPUADDR vertical reload and 240-line nametable wrapping; approximate scanline scheduling')
    p.add_argument('--audio-sweep',action='store_true',help='Opt-in quantized pulse sweep; requires --audio-counters and --experimental-audio')
    p.add_argument('--audio-counters',action='store_true',help='Opt-in frame-quantized envelopes, note lengths and triangle linear counter; requires --experimental-audio')
    ...
    p.add_argument('--no-word-oam',action='store_true',help='Retain the byte block-move OAM copy for regression comparisons')
    p.add_argument('--no-quick-indexed',action='store_true',help='Use the generic handler for indexed absolute memory accesses')
